...
bool on_parameter_changed(const Parameter *old_param, const Parameter *new_param, void *context)
{
	...
	if (strcmp(new_param->name.data, LED_PARAM) == 0) {
		...
		led_brihtness = int32_constrain(led_brihtness, 0, 100);
		...
		servo_angle = int32_constrain(servo_angle, -90, 90);
		osSemaphoreRelease(servo_semHandle);
	}
	return true;
}
...
static void param_task(void *argument)
{
	...
	for (;;) {
		osDelay(10);
		rclc_executor_spin_some(&param_executor, 10 * (1000 * 1000));
	}
}
