...
Aus::Aus(serial::Serial* SerialPtr  ,uint8_t id = 0x00)
{
...
/**
  ...
  * @retval         计算完的校验和
  */
uint8_t Aus::get_check_sum(uint8_t *pch_message,uint32_t dw_length)
{
   uint8_t check_sum = 0;
    if (pch_message == NULL)
    {
...
* @retval			返回要发送的数据大小
*/
uint16_t Aus::send_servo_pack(uint8_t cmd_type, uint8_t *p_data, uint8_t len)
{
 ...
 * @retval none
*/
void Aus::set_servo_angle(int16_t angle_set, uint16_t time_ms)
{
 ...
 * @retval none
*/
void Aus::set_servo_angle(int16_t angle_set)
{
    if(angle_set<=180 && angle_set > -180){
        //单圈模式
...
void Aus::set_servo_angle_multi_turn(int angle_set)
{
...
void Aus::set_servo_angle_multi_turn(int angle_set, unsigned int time_ms)
{
    uint8_t data[11];
    data[0] = servo_id;
    //角度
 ...
 * @retval none
*/
int16_t Aus::get_servo_angle(void)
{
    uint8_t data[1];
    data[0] = servo_id;  
 ...
 * @retval none
*/
void Aus::modify_servo_id(uint8_t new_id)
{
    uint8_t data[3];
    data[0] = servo_id;  
    data[1] = 0x22; 
